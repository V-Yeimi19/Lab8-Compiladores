...
int TypeCheckerVisitor::visit(BreakStm* stm) {
    return 0;
}
...
// -----------------------------------------------------------------------------
// visit(UnaryExp) — emite NOT lógico: sete si %rax == 0
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// visit(DoWhileStm) — emite bucle do-while: ejecuta cuerpo primero, luego evalúa
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// visit(BreakStm) — salta al final del loop más cercano
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// visit(SwitchStm) — emite bloque switch/case con etiquetas únicas
// Algoritmo: evalúa condición, la guarda en %r10, compara con cada case,
// salta a la etiqueta del case si coincide, sino va a default.
// -----------------------------------------------------------------------------
...
int GenCodeVisitor::visit(SwitchStm* stm) {
    ...
    // Evaluar condición y guardarla en %r10
    stm->condition->accept(this);
    out << " movq %rax, %r10\n";
    ...
    for (int i = 0; i < nCases; i++) {
        ...
        out << " cmpq %rax, %r10\n";
        out << " je case_" << lbl << "_" << stm->cases[i].value << "\n";
    }

    // Si ningún case coincidió, ir a default
    out << " jmp default_" << lbl << "\n";

    // Etiquetas de cases con sus bodies
    for (int i = 0; i < nCases; i++) {
        out << "case_" << lbl << "_" << stm->cases[i].value << ":\n";
